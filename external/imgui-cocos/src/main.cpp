...
// Patched so insertText/keyDown/keyUp match the current Geode bindings
// (they gained an extra trailing parameter). Only those three signatures were
// changed; the bodies ignore the added argument. Everything else is upstream.
...
void ImGuiNode::insertText(const char* text, int len, cocos2d::enumKeyCodes) {
...
void ImGuiNode::keyDown(cocos2d::enumKeyCodes key, double) {
	ImGui::GetIO().AddKeyEvent(cocos_to_imkey(key), true);
}

void ImGuiNode::keyUp(cocos2d::enumKeyCodes key, double) {
